function Verified (cb){
    ...
    this.phoneContent=function(){
        ...
        var pTxt='<p class="tip-verified-gb">根据《网络游戏管理暂行办法》规定，需要提供真实姓名以及身份证信息，不通过认证将无法进行充值，我们会绝对保证您的资料安全。</p>';
    ...
    this.renderUI=function(type){
        type ? this.rootNode.html( this.hd + this.tabs + this.main + this.btn ) : this.rootNode.html( this.hd + this.main + this.btn );
    ...
    this.getPicCode=function(){
        $.ajax({
            url:'http://api.wan.liebao.cn/mobile/1/api/validimg',
            dataType :'jsonp',
    ...
    this.getMsg=function(){
        ...
        $.ajax({
            url:'http://api.wan.liebao.cn/mobile/1/api/mobilebindsms?mobile='+m+'&vercode='+c,
            dataType :'jsonp',
            success:function(res){
                if(res.ret==10006){
    ...
    this.phoneBind=function(m,msg){
        $.ajax({
            url:'http://api.wan.liebao.cn/mobile/1/api/mobilebind?mobile='+m+'&code='+msg,
            dataType :'jsonp',
            success:function(res){
                if(res.ret!=1){
