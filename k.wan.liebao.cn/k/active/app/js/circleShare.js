...
var getCircle = {
    init:function () {
        $.ajax({
            // url: commonUrl+'/cmauto/circle/get_article',
            url:'http://10.20.221.60/cmauto//circle/jsonp_get_article?article_id=1',
            // data:{'article_id':id},
            dataType: "jsonp",
            type:'GET',
            cache:true,
        }).success(function(data) {
            $('.circle_top .section img').attr('src',data.data.author_avatar);
            $('.circle_top .auth h3').text(data.data.author_name);
            $('.circle_top .auth h4').text(data.data.author_name);
            $('.circle_content').html(data.data.content);
            $('.circle_footer .circle').text(data.data.circle_name);
            $('.circle_footer .time').text(formatTimeStamp(data.data.publish_ts));
            $('.circle_zan_top p strong').text(data.data.num_like);
            var str='';
            for(var i=0;i<data.data.liked_users.length;i++){
                str+='<li><img class="avatar" src="'+data.data.liked_users[i].avatar+'"><img class="title" src="../img/v.png"></li>'
            }
            $('.circle_zan_list ul').append(str);
            console.log(data);
            // if(data.data) {
            //     $('title').html(data.data.title);
            //     var interText = doT.template($("#J_article_temp").text());
