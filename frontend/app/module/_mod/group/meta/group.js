...
(function() {
  ...
  m.inputData = {
    prop: [_ep('title'), _ep('code'), _ep('val')]
  };
  ...
  m.groupMember = {
    prop: [
      ...
      }), _ep('user:status'), _ep('row'), _ep('introduction')
    ],
    listOpt: {
      colNum: 4
    }
  ...
  m.group = {
    prop: [
      m._link('title'), _ep('subTitle'), m._cat('group'), m._textarea('idea'), _ep('content:content'), m._markdown('terms'), m._markdown('taskIntro'), m._itemTable('task', {
        attrs: {
          btns: ['inlineAdd'],
          itemBtns: ['inlineEdit', 'formDel']
        }
      }), _ep('startedDate'), m._number('week', {
        label: '时长',
        group: {
          suf: '周'
        }
      ...
      }), _ep('content:status'), m._checkbox('applyEnd', {
        label: '停止申请'
    ...
    listOpt: {
      ...
      afterShow: function(e, p) {
        var gid, group;
        group = this.findData(e);
        gid = group.id;
        return app.dm.tb(p, 'groupMember', {
          style: 'panel-success',
          btns: ['popAdd'],
